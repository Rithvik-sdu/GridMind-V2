...
for subsec in range(len(demand_log)):
    ...
    prev_grid_ramp = abs(last_grid_output - prev_grid_output)
    if prev_grid_ramp >= grid_ramp_threshold:
        battery_output_w += increment_1
    else:
        battery_output_w -= increment_1

    if battery_output_w > high_tier_1:
        battery_output_w = high_tier_1
    elif battery_output_w < low_tier_1:
        battery_output_w = low_tier_1
    else:
    ...
    if leftover > 0 and demand_log[subsec] > baseline_demand_w:
        ...
        if (subsec) - 1 < 0:
            ...
            prev_grid_output = grid_output_log[subsec - 1]
